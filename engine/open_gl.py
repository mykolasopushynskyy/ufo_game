...
class OpenGLGame:
    def __init__(
        ...
        self.display = init_display(width, height)
    ...
    def surf_to_texture(self):
        texture = self.ctx.texture(self.display.get_size(), 4)
        texture.filter = (moderngl.NEAREST, moderngl.NEAREST)
        texture.swizzle = "BGRA"
        texture.write(self.display.get_view("1"))
        return texture

    def init_texture(self, **kwargs):
        self.frame_tex = self.surf_to_texture()
